...
- **[Phase 2]** ジェスチャー検知時に、OSレベルでキー入力を送信する。
    ...
    - **状態保持とチャタリング防止 (State Retention & Debounce)**:
        - ジェスチャー確定後、一定時間（クールダウン期間）は認識状態を保持し、視覚的に **Progress Ring** でその残り時間を示す。
        - **同一認知のブロック**: クールダウン期間中、前回と同じジェスチャー（例：YESの後にYES）が検知されても、新たな入力送信は行わない（チャタリング防止）。
        - **異なる認知の割り込み**: クールダウン期間中であっても、前回と異なるジェスチャー（例：YESの後にNO）が検知された場合は、即座にクールダウンをリセットし、新しい状態への遷移と入力送信を行う。
