...
### 3.1. 顔認識・ジェスチャー判定
- カメラ映像からのリアルタイム顔認識。
- **実装技術**: OpenCV (Rustバインディング `opencv` クレート) で顔検出・追跡を行う。
...
- **[Phase 2]** ジェスチャー検知時に、OSレベルでキー入力を送信する。
    ...
    - **音声入力・コマンド (Voice & Desktop Synergy) [Phase 2.5]**:
...
### 3.4. GUIアプリケーション (共通)
- **実装技術**: `eframe` (`egui`)。
- **デバイス切り替え**: 複数のカメラ・マイク（将来用）をドロップダウンで即座に切り替え可能にする。
- **プライバシー保護プレビュー**:
    - **[Phase 1]**: 顔検出領域以外をぼかす、または塗りつぶして表示する機能。
    - **[Phase 2]**: AIによる高精度な背景除去・差し替え機能。
- 現在の判定状態（YES/NO/UNKNOWN）の視覚的表示（スタンプ風アニメーション等）。

### 3.5. MCPサーバー機能
- **[Phase 3]** AIエージェントからの「現在の判定状態」の問い合わせに応答する機能の追加。

## 4. 非機能要件
- **安全性**: 画像データをメモリセーフに扱い、外部送信を行わない。
- **ポータビリティ**: シングルバイナリ配布（Native版）と、ブラウザ拡張機能（WASM版）の両立を考慮した設計。
