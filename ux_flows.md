...
## 8. ウィンドウ管理フロー (Desktop)
1.  **ホバー**: ユーザーがウィジェットにマウスカーソルを移動。
2.  **操作UI表示**: 右上に「X（閉じる）」、中央に「ドラッグハンドル」が表示される。
3.  **移動**: ドラッグハンドルまたは背景をドラッグして、ウィジェットを移動。
4.  **終了**: 「X」ボタンをクリックしてアプリケーションを終了。
5.  **フェードアウト**: マウスを離すと、操作UIが消え、再びミニマルなアバター表示に戻る。
